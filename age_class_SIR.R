...
## draw box for plot:
plot(0,0,xlim=c(0,tmax),ylim=c(0,0.02),
...
draw.soln <- function(ic=c(S1,S2,I1,I2,R1,R2), tmax=1,
  ...
  lines(times, soln[,"I1"], col = c("blue","red")[i], lwd=2,... )
  lines(times, soln[,"I2"], col = c("purple","black")[i], lwd=2,... )
...
legend("topright", c("Low Contact", "Low Contact EF", "High Contact", "High Contact EF"), col = c("blue", "red", "purple", "black"), lwd = c(2,2,2,2))